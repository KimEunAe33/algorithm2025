...
def tournament_sort(arr, stability=lambda x:x[0]):
    n = len(arr)
    leaf_size = 1
    while leaf_size < n:
        leaf_size *= 2
    tree = [[float('inf'), -1]] * (2 * leaf_size - 1)
    ...
    for i in range(leaf_size - 2, -1, -1):
        left = tree[2 * i + 1]
        right = tree[2 * i + 2]
        tree[i] = min(left, right, key=stability) 

    for j in range(n):
        min_index = 0
        for i in range(n):
            if stability(tree[leaf_size-1 + i]) == stability(tree[min_index]):
                min_index =  leaf_size-1 + i
        
        arr[j]=tree[min_index]
        tree[min_index] = [float('inf'), -1]  

        while min_index > 0:
            parent_index = (min_index - 1) // 2
            left = tree[2 * parent_index + 1]
            right = tree[2 * parent_index + 2]
            tree[parent_index] = min(left,right, key=stability) 
